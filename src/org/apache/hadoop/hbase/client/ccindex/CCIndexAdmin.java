...
/**
 * Extension of HBaseAdmin that creates index tables and check tables.
 * 
 */

public class CCIndexAdmin extends HBaseAdmin {

	private IndexedTableDescriptor des;
  private static final Log LOG = LogFactory.getLog(CCIndexAdmin.class);
  public static final byte[] CCT_TAIL=Bytes.toBytes("_CCT");
  public static final byte[] INDEX_TAIL=Bytes.toBytes("_INDEX");
  /**
   * Constructor
   * 
   ...
   */
  public CCIndexAdmin(HBaseConfiguration conf)
      throws MasterNotRunningException {
    super(conf);
  }
  ...
  public void createIndexedTable(IndexedTableDescriptor desc) throws IOException {
    ...
    HTableDescriptor baseCCT=this.createCCTBase(desc.getBaseTableDescriptor().getName());
    super.createTable(baseCCT);
  }
  ...
  private void createIndexTables(IndexedTableDescriptor indexDesc) throws IOException {
    ...
    for (IndexSpecification indexSpec : indexDesc.getIndexes()) {
      ...
      HTableDescriptor CCTDesc = this.createCCTDesc(baseTableName,
              indexSpec);
  ...
  private HTableDescriptor createCCTDesc(byte[] baseTableName,
	      IndexSpecification indexSpec) throws ColumnNameParseException {
	    HTableDescriptor indexTableDesc = new HTableDescriptor(
	    		Bytes.add(indexSpec
	    		        .getIndexedTableName(baseTableName), this.CCT_TAIL)
	    	);
	    Set<byte[]> families = new TreeSet<byte[]>(Bytes.BYTES_COMPARATOR);
	    
	   for(IndexSpecification spec:this.des.getIndexes())
	   {
		   for (byte[] column : spec.getIndexedColumns()) {
			      families.add(Bytes.add(HStoreKey.getFamily(column),
			          new byte[] { HStoreKey.COLUMN_FAMILY_DELIMITER }));
			    }
	   }
  ...
  private HTableDescriptor createCCTBase(byte[] baseTableName
	      ) throws ColumnNameParseException {
	    HTableDescriptor indexTableDesc = new HTableDescriptor(
	    		Bytes.add(baseTableName, this.CCT_TAIL)
	    	);
	    Set<byte[]> families = new TreeSet<byte[]>(Bytes.BYTES_COMPARATOR);
