...
public class IndexedTable extends TransactionalTable {
	...
	public static final byte[] INDEX_COL_FAMILY_NAME = Bytes
			.toBytes("__INDEX__");
	...
	private HashMap<String, String> columnToIndexID = new HashMap<String, String>();
	...
	public IndexedTable(final HBaseConfiguration conf, final byte[] tableName)
		...
		if (refiner == null) {
			refiner = new SimpleRefiner(conf, this);
			refiner.start();
		}
		this.indexedTableDescriptor = new IndexedTableDescriptor(super
				.getTableDescriptor());
		for (IndexSpecification spec : this.indexedTableDescriptor.getIndexes()) {
			...
			if (tableName != null) {
				indexIdToTable.put("indexed-org", new HTable(conf, tableName));
			}
		}
	...
	public HashMap<String, String> getColumnToIndexID() {
		return columnToIndexID;
	}

	public void setColumnToIndexID(HashMap<String, String> columnToIndexID) {
		this.columnToIndexID = columnToIndexID;
	}

	public Map<String, HTable> getIndexIdToTable() {
		return indexIdToTable;
	}

	public void setIndexIdToTable(Map<String, HTable> indexIdToTable) {
	...
	public void initColumnIndexMap() {
		...
		for (IndexSpecification index : col) {
			byte[][] b = index.getIndexedColumns();
			for (byte[] column : b) {
				this.columnToIndexID
						.put(new String(column), index.getIndexId());
			}
		}
	}

	public static SimpleRefiner refiner;
	...
	private SingleReader getSingleReader(Range[] range) {
		...
		String indexId = this.columnToIndexID.get(new String(range[flag]
				.getColumn()));
	...
	public ResultScanner getIndexedScanner(String indexId,
		...
		if (indexStopRow != null) {
			end = Bytes.add(indexStopRow, this.maxRowKey);
		}
	...
	public ResultScanner getIndexedScanner(String indexId,
		...
		// allIndexColumns = new byte[indexColumns.length +
		// baseColumns.length+1][];
		// System
		// .arraycopy(indexColumns, 0, allIndexColumns, 0, indexColumns.length);
		// System
		// .arraycopy(baseColumns, 0, allIndexColumns, indexColumns.length,
		// baseColumns.length);
		// allIndexColumns[indexColumns.length + baseColumns.length] =
		// INDEX_BASE_ROW_COLUMN;
		// } else {
		// byte[][] allColumns = indexSpec.getAllColumns();
		//     
		// allIndexColumns = new byte[allColumns.length + 1][];
		// System.arraycopy(allColumns, 0, allIndexColumns, 0,
		// allColumns.length);
		...
		indexScan.addColumn(INDEX_BASE_ROW_COLUMN);
	...
	public class ScannerWrapper implements ResultScanner {
		...
		private HashMap<byte[], Boolean> col;
		...
		public HashMap<byte[], Boolean> getColumnMap(byte[][] col) {
			if (col == null) {
				return null;
			} else {
				HashMap<byte[], Boolean> ret = new HashMap<byte[], Boolean>();
				for (byte[] b : col) {
					ret.put(b, true);
				}
				return ret;
		...
		/** {@inheritDoc} */
		/**
		 * add by liujia not going to base table to find values
		 */
		private int log = 0;

		public Result[] nextOrg(int nbRows) {
			Result[] indexResult = null;
			try {
				indexResult = indexScanner.next(nbRows);
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				LOG.error("index table scan errror");
			}
			...
			for (int i = 0; i < indexResult.length; i++) {
				...
				if (baseRow == null) {
					LOG.error("index table row don't have base column:"
							+ new String(INDEX_BASE_ROW_COLUMN));
					return null;
				...
				log++;
				if (log % 1000 == 0) {
					LOG.debug("sequence:" + log + "next index row ["
							+ Bytes.toString(row.getRow()) + "] -> base row ["
				...
				if (columns != null && columns.length > 0) {
					...
					try {
						...
						// TODO Auto-generated catch block
						LOG.error("org table don't have this row :" + row);
					}
				}
				...
				for (KeyValue indexKV : row.list()) {
					...
					results.add(new KeyValue(baseRow, indexKV.getColumn(),
							indexKV.getTimestamp(), KeyValue.Type.Put, indexKV
									.getValue()));
				}
				...
				if (baseResult != null) {
					try {
						...
						// TODO Auto-generated catch block
						LOG.error("add org table value error");
					}
				}
