...
public class SimpleIndexKeyGenerator implements IndexKeyGenerator {
	...
	public byte[] createIndexKey(byte[] rowKey, Map<byte[], byte[]> columns) {
		...
		return CreateICTIndexKey(rowKey, columns);
	}

	public byte[] CreateICTIndexKey(byte[] rowKey, Map<byte[], byte[]> columns) {
		byte[] b = Bytes.add(columns.get(column), rowKey);
		// byte [] newKey=new byte[b.length+8];
		long columnLength = columns.get(column).length;
