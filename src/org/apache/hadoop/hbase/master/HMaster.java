...
import org.apache.hadoop.hbase.master.ProcessServerShutdown.ScanMetaRegions;
...
HMasterRegionInterface, Watcher {
	...
	volatile DelayQueue<RegionServerOperation> delayedToDoQueue = new DelayQueue<RegionServerOperation>();
	...
	public HMaster(HBaseConfiguration conf) throws IOException {
		...
		String addressStr = DNS.getDefaultHost(conf.get(
				"hbase.master.dns.interface", "default"), conf.get(
		...
		this.maxRegionOpenTime = conf.getLong(
				"hbase.hbasemaster.maxregionopen", 120 * 1000);
		this.leaseTimeout = conf
		...
		this.metaRescanInterval = conf.getInt(
	...
	/*
	 * Return true if we are the master, false if the cluster must shut down or
	 * if we only retry once.
	 */
	...
	private void bootstrap() throws IOException {
		...
		try {
			...
			// created here in bootstap and it'll need to be cleaned up. Better
			// to
			// not make it in first place. Turn off block caching for bootstrap.
			// Enable after.
			HRegionInfo rootHRI = new HRegionInfo(HRegionInfo.ROOT_REGIONINFO);
			setInfoFamilyCaching(rootHRI, false);
			HRegionInfo metaHRI = new HRegionInfo(
					HRegionInfo.FIRST_META_REGIONINFO);
			setInfoFamilyCaching(metaHRI, false);
			HRegion root = HRegion.createHRegion(rootHRI, this.rootdir,
					this.conf);
			HRegion meta = HRegion.createHRegion(metaHRI, this.rootdir,
					this.conf);
	...
	public static class metaChecker extends Thread
	{
		private HMaster master=null;
		public metaChecker(HMaster master)
		{
			this.master=master;
		}
	public void run()
	{
		...
		{
			master.processDeadServerRegionInMeta();
			try {
				sleep(30000);
	...
	public void run() {
		...
		verifyClusterState();
		startServiceThreads();
//		metaChecker checker=null;
//		checker=new metaChecker(this);
//		checker.start();
		/* Main processing loop */
		try {
			while (!closed.get()) {
				...
				if (shutdownRequested.get()) {
					// The region servers won't all exit until we stop scanning
					// the
					// meta regions
...
//		}
	...
	private boolean processToDoQueue() {
		...
		if (regionManager.getRootRegionLocation() != null) {
			// We can't process server shutdowns unless the root region is
			// online
		...
		// at this point, if there's still no todo operation, or we're supposed
		// to
		// be closed, return.
		...
		try {
			...
			if (!op.process()) {
				...
				// online. This could cause a deadlock, because this thread is
				// waiting
				// for the missing meta region(s) to come back online, but since
				// it
				// is waiting, it cannot process the meta region online
				// operation it
				// is waiting for. So put this operation back on the queue for
				// now.
				if (toDoQueue.size() == 0) {
					// The queue is currently empty so wait for a while to see
					// if what
			...
			if (ex instanceof RemoteException) {
				try {
					...
					.decodeRemoteException((RemoteException) ex);
				} catch (IOException e) {
					ex = e;
	...
	private void verifyClusterState() {
		try {
			...
			HServerAddress rootLocation = zooKeeperWrapper
			...
			if (addresses.size() == 0) {
				LOG
				.debug("This is a fresh start, proceeding with normal startup");
			...
			// - contact every region server to add them to the regionservers
			// list
			...
			for (HServerAddress address : addresses) {
				...
				for (HRegionInfo region : regions) {
					if (region.isRootRegion()) {
						connection.setRootRegionLocation(new HRegionLocation(
								region, rootLocation));
						regionManager.setRootRegionLocation(rootLocation);
						// Undo the unassign work in the RegionManager
						// constructor
						regionManager.removeRegion(region);
						isRootRegionAssigned = true;
					} else if (region.isMetaRegion()) {
						MetaRegion m = new MetaRegion(new HServerAddress(
								address), region);
			...
			LOG.info("Inspection found "
					...
					+ " regions, "
					+ (isRootRegionAssigned ? "with -ROOT-"
							: "but -ROOT- was MIA"));
			splitLogAfterStartup();
	...
	/**
	 * Inspect the log directory to recover any log file without and active
	 * region server.
	 * 
	 * @throws IOException
	 */
	private void splitLogAfterStartup() throws IOException {
		Path logsDirPath = new Path(this.rootdir,
		...
		for (FileStatus status : logFolders) {
			...
			if (this.serverManager.getServerInfo(serverName) == null) {
				...
				Path logDir = new Path(this.rootdir, HLog
						.getHLogDirectoryName(serverName));
				try {
	 ...
	 * exception then they just die with a logged message. This should be fine
	 * because in general, we do not expect the master to get such unhandled
	 * exceptions as OOMEs; it should be lightly loaded. See what HRegionServer
	...
	private void startServiceThreads() {
		...
		try {
			...
			if (port >= 0) {
				String a = this.conf.get("hbase.master.info.bindAddress",
				"0.0.0.0");
	...
	public static HashMap<String ,HashSet<String>> noServedRegion=new HashMap<String ,HashSet<String>> ();
	public HMsg[] regionServerReport(HServerInfo serverInfo, HMsg msgs[],
			HRegionInfo[] mostLoadedRegions) throws IOException {

		
		if(msgs!=null&&msgs.length>=1&&msgs[0].isType(HMsg.Type.MSG_NO_SERVER_REGION))
		{
			
			if(this.noServedRegion.get(serverInfo.getServerAddress().toString())==null)
			{
				this.noServedRegion.put(serverInfo.getServerAddress().toString(), new HashSet<String>());
				
			}
			synchronized (this.noServedRegion)
			{
				
				this.noServedRegion.get(serverInfo.getServerAddress().toString()).add(new String(msgs[0].getRegionInfo().getRegionName()));
			}
			return new HMsg[0];
		}
		else
	...
	public void createTable(HTableDescriptor desc) throws IOException {
		...
		for (int tries = 0; tries < numRetries; tries++) {
			try {
				// We can not create a table unless meta regions have already
				// been
				...
				if (!this.serverManager.canAssignUserRegions()) {
					...
					"not enough servers to create table yet");
				}
		...
		try {
			...
			if (data != null && data.size() > 0) {
				HRegionInfo info = Writables.getHRegionInfo(data.getValue(
						CATALOG_FAMILY, REGIONINFO_QUALIFIER));
	...
	private List<Pair<HRegionInfo, HServerAddress>> getTableRegions(
		...
		Set<MetaRegion> regions = regionManager
		.getMetaRegionsForTable(tableName);
		byte[] firstRowInTable = Bytes
		.toBytes(Bytes.toString(tableName) + ",,");
		for (MetaRegion m : regions) {
			byte[] metaRegionName = m.getRegionName();
			HRegionInterface srvr = connection.getHRegionConnection(m
					.getServer());
			Scan scan = new Scan(firstRowInTable);
			scan.addColumn(CATALOG_FAMILY, REGIONINFO_QUALIFIER);
			...
			try {
				while (true) {
					...
					HRegionInfo info = Writables.getHRegionInfo(data.getValue(
							CATALOG_FAMILY, REGIONINFO_QUALIFIER));
					if (Bytes.compareTo(info.getTableDesc().getName(),
							tableName) == 0) {
						byte[] value = data.getValue(CATALOG_FAMILY,
								SERVER_QUALIFIER);
						if (value != null) {
							HServerAddress server = new HServerAddress(Bytes
									.toString(value));
					...
					if (Bytes.compareTo(info.getTableDesc().getName(),
						...
						if ((Bytes.compareTo(info.getStartKey(), rowKey) >= 0)
							...
							if (value != null) {
								...
								return new Pair<HRegionInfo, HServerAddress>(
										info, server);
							}
						}
						LOG.info("info.get start key is" + info.getStartKey());
						LOG.info("info.get end key is" + info.getEndKey());
	...
	public void modifyTable(final byte[] tableName, HConstants.Modify op,
		...
		case TABLE_SET_HTD:
			if (args == null || args.length < 1
				...
				throw new IOException(
				"SET_HTD request requires an HTableDescriptor");
		...
		case TABLE_FLUSH:
			if (args != null && args.length > 0) {
				...
				if (tableName == null) {
					byte[] regionName = ((ImmutableBytesWritable) args[0])
					.get();
				...
				if (pair != null) {
					this.regionManager.startAction(pair.getFirst()
		...
		case CLOSE_REGION:
			...
			LOG.debug("Attempting to close region: "
					+ Bytes.toStringBinary(regionname));
			String servername = null;
			if (args.length == 2) {
				servername = Bytes.toString(((ImmutableBytesWritable) args[1])
						.get());
			}
			// Need hri
			...
			if (servername == null) {
				...
				servername = Bytes.toString(rr.getValue(CATALOG_FAMILY,
						SERVER_QUALIFIER));
			}
			// Take region out of the intransistions in case it got stuck there
			// doing
			// an open or whatever.
			...
			long startCode = Bytes.toLong(rr.getValue(CATALOG_FAMILY,
					STARTCODE_QUALIFIER));
	...
	public ClusterStatus getClusterStatus() {
		...
		status.setRegionsInTransition(this.regionManager
		...
		try {
			...
			while ((r = indexScanner.next()) != null) {
				...
				try {
					...
					value = r.getValue(CATALOG_FAMILY, REGIONINFO_QUALIFIER);
					...
					value = r.getValue(CATALOG_FAMILY, SERVER_QUALIFIER);
					...
					if(this.noServedRegion.get(serverAddress)!=null)
					{
						if(this.noServedRegion.get(serverAddress).size()!=0)
						{
							synchronized(this.noServedRegion)
							{
								System.out.println("check server:"+serverAddress);
								for(String b:this.noServedRegion.get(serverAddress))
								{
									System.out.println("contains region:"+b);
								}
								if(this.noServedRegion.get(serverAddress).contains(new String(regionInfo.getRegionName())))
										{
												this.regionManager.setUnassigned(regionInfo, true);
												this.noServedRegion.get(serverAddress).remove(regionInfo.getRegionName());
												System.out.println("check region:"+new String(regionInfo.getRegionName()));
												System.out.println("is contains:"+new String(regionInfo.getRegionName()));
												continue;
										}
							}
					...
					for(String dead:this.serverManager.deadServers)
					{
						LOG.info("dead server is:"+dead);
					}

					if(!this.serverManager.serversToServerInfo.containsKey(serverName))
					{
						LOG.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" +
								"dead server is:"+serverName);
						this.regionManager.setUnassigned(regionInfo, true);	
					}
					if(this.serverManager.deadServers.contains(serverName))
					{
	...
	/*
	 * Get HRegionInfo from passed META map of row values. Returns null if none
	 * found (and logs fact that expected COL_REGIONINFO was missing). Utility
	...
	throws IOException {
		...
		if (regioninfo == null) {
			...
			NavigableMap<byte[], byte[]> infoMap = res
			.getFamilyMap(CATALOG_FAMILY);
			for (byte[] e : infoMap.keySet()) {
				if (sb.length() > 0) {
					sb.append(", ");
				}
				sb.append(Bytes.toString(CATALOG_FAMILY) + ":"
						+ Bytes.toString(e));
			}
			LOG.warn(Bytes.toString(CATALOG_FAMILY) + ":"
					+ Bytes.toString(REGIONINFO_QUALIFIER)
					+ " is empty for row: " + Bytes.toString(row)
					+ "; has keys: " + sb.toString());
			return null;
		}
	...
	public void process(WatchedEvent event) {
		...
		if (event.getState() == KeeperState.Expired
			...
			try {
				...
				// Verify the cluster to see if anything happened while we were
				// away
				verifyClusterState();
			} catch (Exception e) {
	...
	private static void printUsageAndExit() {
		...
		System.err
		.println(" start  Start Master. If local mode, start Master and RegionServer in same JVM");
		System.err
		.println(" stop   Start cluster shutdown; Master signals RegionServer shutdown");
		System.err.println(" where [opts] are:");
		System.err
		.println("   --minServers=<servers>    Minimum RegionServers needed to host user tables.");
		System.exit(0);
	}
	...
	protected static void doMain(String[] args,
		...
		for (String cmd : args) {
			...
			if (cmd.startsWith("--minServers=")) {
				conf.setInt("hbase.regions.server.count.min", Integer
						.valueOf(cmd.substring(13)));
				continue;
			}
			...
			if (cmd.equals("start")) {
				try {
					...
					if (LocalHBaseCluster.isLocal(conf)) {
						...
						Constructor<? extends HMaster> c = masterClass
