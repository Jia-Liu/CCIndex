...
/**
 * Instantiated when a server's lease has expired, meaning it has crashed. The
 * region server's log file needs to be split up for each region it was serving,
 * and the regions need to get reassigned.
 */
class ProcessServerShutdown extends RegionServerOperation {
	...
	public ProcessServerShutdown(HMaster master, HServerInfo serverInfo) {
		...
		this.oldLogDir = new Path(master.rootdir, HLog
				.getHLogDirectoryName(serverInfo));
	...
	private void closeMetaRegions() {
		...
		if (isRootServer) {
			...
			// HBASE-1928: Check whether this server has been transitioning the
			// ROOT table
			isRootServer = master.regionManager
					.isRootServerCandidate(deadServer);
			if (isRootServer) {
				master.regionManager.unsetRootRegion();
			}
		}
		List<byte[]> metaStarts = master.regionManager
				.listMetaRegionsForServer(deadServerAddress);
		...
		// HBASE-1928: Check whether this server has been transitioning the META
		// table
		HRegionInfo metaServerRegionInfo = master.regionManager
				.getMetaServerRegionInfo(deadServer);
		if (metaServerRegionInfo != null) {
			metaRegions.add(new MetaRegion(deadServerAddress,
					metaServerRegionInfo));
		}
	}
	 ...
	 */
	protected static void scanMeta(HRegionInterface server, long scannerId,
			byte[] regionName, HMaster master) throws IOException {
		...
		try {
			while (true) {
				...
				// Check server name. If null, skip (We used to consider it was
				// on
				// shutdown server but that would mean that we'd reassign
				// regions that
				// were already out being assigned, ones that were product of a
				// split
				...
				synchronized (master.regionManager) {
					...
					if (master.regionManager.isOfflined(info
			...
			if (scannerId != -1L) {
				try {
					...
					LOG.error("Closing scanner", RemoteExceptionHandler
							.checkIOException(e));
				}
			}
		}
		...
		try {
			while (true) {
				...
				if (LOG.isDebugEnabled() && row != null) {
					LOG.debug("Shutdown scanner for " + serverName
							+ " processing " + Bytes.toString(row));
				}

				// HRegionInfo info = master.getHRegionInfo(row,
				// values.rowResult());
				HRegionInfo info = master.getHRegionInfo(row, values);
				if (info == null) {
					emptyRows.add(row);
				...
				synchronized (master.regionManager) {
					if (info.isMetaTable()) {
						...
						master.regionManager.offlineMetaRegion(info
								.getStartKey());
					}
	...
	private class ScanRootRegion extends RetryableMetaOperation<Boolean> {
		...
		public Boolean call() throws IOException {
			...
			long scannerId = server.openScanner(HRegionInfo.ROOT_REGIONINFO
					.getRegionName(), scan);
			scanMetaRegion(server, scannerId, HRegionInfo.ROOT_REGIONINFO
					.getRegionName());
			return true;
		}
	}

	public static class ScanMeta extends RetryableMetaOperation<Boolean> {
		ScanMeta(MetaRegion m, HMaster master) {
			super(m, master);
		}
		...
		public Boolean call() throws IOException {
			if (LOG.isDebugEnabled()) {
				LOG.debug("process server shutdown scanning "
						+ Bytes.toString(m.getRegionName()) + " on "
						+ m.getServer());
			}
			Scan scan = new Scan();
			scan.addFamily(CATALOG_FAMILY);
			...
			LOG.info("thread is:" + Thread.currentThread().getName()
					+ " m region is:" + m.toString());
			ProcessServerShutdown.scanMeta(server, scannerId,
					m.getRegionName(), this.master);
			return true;
		}
	}

	public class ScanMetaRegions extends RetryableMetaOperation<Boolean> {
	...
	protected boolean process() throws IOException {
		LOG.info("process shutdown of server " + this.deadServer
				+ ": logSplit: " + logSplit + ", rootRescanned: "
		...
		if (!logSplit) {
			...
			if (master.fs.exists(oldLogDir)) {
				...
				try {
					HLog.splitLog(master.rootdir, oldLogDir, master.fs, master
							.getConfiguration());
				} finally {
		...
		if (!rootRescanned) {
			...
			Boolean result = new ScanRootRegion(new MetaRegion(master
		...
		List<MetaRegion> regions = master.regionManager
				.getListOfOnlineMetaRegions();
		for (MetaRegion r : regions) {
			Boolean result = new ScanMetaRegions(r, this.master)
					.doWithRetries();
			if (result == null) {
				break;
			}
			if (LOG.isDebugEnabled()) {
				LOG.debug("process server shutdown finished scanning "
						+ Bytes.toString(r.getRegionName()) + " on "
						+ r.getServer());
			}
		}
