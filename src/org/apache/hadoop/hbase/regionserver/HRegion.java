 ...
 * 
 * <p>
 * An Store is a set of rows with some column data; together, they make up all
 ...
 * The startKey for the first region is null. The endKey for the final region is
 * null.
 * 
 * <p>
 * Locking at the HRegion level serves only one purpose: preventing the region
 * from being closed (and consequently split) while other operations are
 * ongoing. Each row level operation obtains both a row lock and a region read
 * lock for the duration of the operation. While a scanner is being constructed,
 * getScanner holds a read lock. If the scanner is successfully constructed, it
 * holds a read lock until it is closed. A close takes out a write lock and
 * consequently will block for ongoing operations and will block new operations
 * from starting while the close is in progress.
 * 
 * <p>
 * An HRegion is defined by its table and its key extent.
 * 
 * <p>
 * It consists of at least one Store. The number of Stores should be
 * configurable, so that data which is accessed together is stored in the same
 * Store. Right now, we approximate that by building a single Store for each
 * column family. (This config info will be communicated via the tabledesc.)
 * 
 * <p>
 * The HTableDescriptor contains metainfo about the HRegion's table. regionName
 * is a unique identifier for this HRegion. (startKey, endKey] defines the
...
public class HRegion implements HConstants, HeapSize { // , Writable{
	...
	/*
	 * Closing can take some time; use the closing flag if there is stuff we
	 * don't want to do while in closing state; e.g. like offer this region up
	 * to the master as a region to close if the carrying regionserver is
	 * overloaded. Once set, it is never cleared.
	 */
	...
	// These variable are just used for getting data out of the region, to test
	// on
	// client side
	...
	protected final ReadWriteConsistencyControl rwcc = new ReadWriteConsistencyControl();
	...
	public final static byte[] REGIONINFO_FILE_BYTES = Bytes
			.toBytes(REGIONINFO_FILE);
	 ...
	 * {@link org.apache.hadoop.hbase.regionserver.HRegion#newHRegion(org.apache.hadoop.fs.Path, HLog, org.apache.hadoop.fs.FileSystem, org.apache.hadoop.hbase.HBaseConfiguration, org.apache.hadoop.hbase.HRegionInfo, FlushRequester)}
	 * method.
	 * 
	 * @param basedir
	 ...
	 *            (There's a single HLog for all the HRegions on a single
	 *            HRegionServer.) The log file is a logfile from the previous
	 *            execution that's custom-computed for this HRegion. The
	 *            HRegionServer computes and sorts the appropriate log info for
	 *            this HRegion. If there is a previous log file (implying that
	 *            the HRegion has been written-to before), then read it from the
	 *            supplied path.
	 * @param fs
	 *            is the filesystem.
	 * @param conf
	 ...
	 * @param regionInfo
	 *            - HRegionInfo that describes the region is new), then read
	 *            them from the supplied path.
	 * @param flushListener
	 *            an object that implements CacheFlushListener or null making
	 *            progress to master -- otherwise master might think region
	 *            deploy failed. Can be null.
	 * 
	 * @see org.apache.hadoop.hbase.regionserver.HRegion#newHRegion(org.apache.hadoop.fs.Path,
	 *      HLog, org.apache.hadoop.fs.FileSystem,
	 *      org.apache.hadoop.hbase.HBaseConfiguration,
	 *      org.apache.hadoop.hbase.HRegionInfo, FlushRequester)
	 */
	public HRegion(Path basedir, HLog log, FileSystem fs,
			HBaseConfiguration conf, HRegionInfo regionInfo,
		...
		this.threadWakeFrequency = conf.getLong(THREAD_WAKE_FREQUENCY,
				10 * 1000);
		String encodedNameStr = Integer.toString(this.regionInfo
				.getEncodedName());
		...
		this.regionCompactionDir = new Path(getCompactionDir(basedir),
				encodedNameStr);
	...
	/**
	 * Initialize this region and get it ready to roll. Called after
	 * construction.
	 * 
	 * @param initialFiles
	 *            path
	...
	public void initialize(Path initialFiles, final Progressable reporter)
		...
		for (HColumnDescriptor c : this.regionInfo.getTableDesc().getFamilies()) {
			try {
				Store store = instantiateHStore(this.basedir, c, oldLogFile,
						reporter);
				...
				long storeSeqIdBeforeRecovery = store
						.getMaxSeqIdBeforeLogRecovery();
	...
	boolean hasReferences() {
		for (Map.Entry<byte[], Store> e : this.stores.entrySet()) {
			for (Map.Entry<Long, StoreFile> ee : e.getValue().getStorefiles()
					.entrySet()) {
				// Found a reference, return.
				if (ee.getValue().isReference())
					return true;
			}
		}
		return false;
	...
	private void checkRegioninfoOnFilesystem() throws IOException {
		// Name of this file has two leading and trailing underscores so it
		// doesn't
		// clash w/ a store/family name. There is possibility, but assumption is
		// that its slim (don't want to use control character in filename
		// because
		// 
	 ...
	 *         HStores make use of. It's a list of all HStoreFile objects.
	 ...
	 * @return Vector of all the storage files that the HRegion's component
	 *         HStores make use of. It's a list of HStoreFile objects. Can be
	 *         null if we are not to close at this time or we are already
	 *         closed.
	 * 
	 * @throws IOException
	 *             e
	...
	public List<StoreFile> close(final boolean abort) throws IOException {
		...
		synchronized (splitLock) {
			...
			synchronized (writestate) {
				// Disable compacting and flushing by background threads for
				// this
				// region.
					...
					LOG
							.debug("waiting for"
									+ (writestate.compacting ? " compaction"
											: "")
									+ (writestate.flushing ? (writestate.compacting ? ","
											: "")
											+ " cache flush"
											: "") + " to complete for region "
									+ this);
					try {
						writestate.wait();
			...
			try {
				...
				LOG
						.debug("Updates disabled for region, no outstanding scanners on "
								+ this);
				try {
					// Write lock means no more row locks can be given out. Wait
					// on
					// outstanding row locks to come in before we close so we do
					// not drop
					// outstanding updates.
					waitOnRowLocks();
					LOG
							.debug("No more row locks outstanding on region "
									+ this);
	...
	private boolean worthPreFlushing() {
		return this.memstoreSize.get() > this.conf.getLong(
				"hbase.hregion.preclose.flush.size", 1024 * 1024 * 5);
	}
	...
	// HRegion maintenance.
	//
	// These methods are meant to be called periodically by the HRegionServer
	// for
	// upkeep.
	// ////////////////////////////////////////////////////////////////////////////
	...
	/*
	 * Split the HRegion to create two brand-new ones. This also closes current
	 * HRegion. Split should be fast since we don't rewrite store files but
	 * instead create new 'reference' store files that read off the top and
	...
	HRegion[] splitRegion(final byte[] splitRow) throws IOException {
		...
		synchronized (splitLock) {
			...
			LOG.fatal("!!!!!!!!!!!!!!!!!splitRegion!!!!!!!!!!!!!!!!");
			LOG.fatal("Start Key is: " + new String(startKey));
			LOG.fatal("End Key is:" + new String(endKey));
			LOG.fatal("splitRow is:" + new String(splitRow));
			...
			HRegionInfo regionAInfo = new HRegionInfo(this.regionInfo
					.getTableDesc(), startKey, splitRow, false, rid);
			Path dirA = getSplitDirForDaughter(splits, regionAInfo);
			HRegionInfo regionBInfo = new HRegionInfo(this.regionInfo
					.getTableDesc(), splitRow, endKey, false, rid);
			Path dirB = getSplitDirForDaughter(splits, regionBInfo);

			// Now close the HRegion. Close returns all store files or null if
			// not
			...
			if (hstoreFilesToSplit == null) {
				...
				throw new RuntimeException(
						"close returned empty vector of HStoreFiles");
			}

			// Split each store file.
			for (StoreFile h : hstoreFilesToSplit) {
				StoreFile.split(fs, Store.getStoreHomedir(splits, regionAInfo
						.getEncodedName(), h.getFamily()), h, splitRow,
						Range.bottom);
				StoreFile.split(fs, Store.getStoreHomedir(splits, regionBInfo
						.getEncodedName(), h.getFamily()), h, splitRow,
						Range.top);
			}

			// Create a region instance and then move the splits into place
			// under
			// regionA and regionB.
			HRegion regionA = HRegion.newHRegion(basedir, log, fs, conf,
					regionAInfo, null);
			moveInitialFilesIntoPlace(this.fs, dirA, regionA.getRegionDir());
			HRegion regionB = HRegion.newHRegion(basedir, log, fs, conf,
					regionBInfo, null);
	...
	private Path getSplitDirForDaughter(final Path splits, final HRegionInfo hri)
			throws IOException {
		Path d = new Path(splits, Integer.toString(hri.getEncodedName()));
		if (fs.exists(d)) {
			// This should never happen; the splits dir will be newly made when
			// we
			// come in here. Even if we crashed midway through a split, the
			// reopen
	...
	/*
	 * Do preparation for pending compaction. Clean out any vestiges of previous
	 * failed compactions.
	 * 
	 * @throws IOException
	 */
	...
	byte[] compactStores(final boolean majorCompaction) throws IOException {
		if (this.closing.get() || this.closed.get()) {
			LOG.debug("Skipping compaction on " + this
			...
			try {
				synchronized (writestate) {
					if (!writestate.compacting && writestate.writesEnabled) {
						...
						LOG
								.info("NOT compacting region " + this
										+ ": compacting="
				...
				doRegionCompactionCleanup();
				String timeTaken = StringUtils.formatTimeDiff(System
	 ...
	 *             Thrown when replay of hlog is required because a Snapshot was
	 ...
	 * process reads/writes as much as possible during the flush operation.
	 * Also, the log has to state clearly the point in time at which the
	 * memstore was flushed. (That way, during recovery, we know when we can
	 * rely on the on-disk flushed structures and when we have to recover the
	 * memstore from the log.)
	 * 
	 * <p>
	 * So, we have a three-step process:
	 * 
	 * <ul>
	 * <li>A. Flush the memstore to the on-disk stores, noting the current
	...
	protected boolean internalFlushcache() throws IOException {
		...
		// Stop updates while we snapshot the memstore of all stores. We only
		// have
		...
		List<StoreFlusher> storeFlushers = new ArrayList<StoreFlusher>(stores
				.size());
		try {
			sequenceId = log.startCacheFlush();
			completeSequenceId = this
					.getCompleteCacheFlushSequenceId(sequenceId);

			for (Store s : stores.values()) {
				storeFlushers.add(s.getStoreFlusher(completeSequenceId));
			}

			// This thread is going to cause a whole bunch of scanners to
			// reseek.
			// They are depending
			// on a thread-local to know where to read from.
			// The reason why we set it up high is so that each HRegionScanner
			// only
		...
		// restart so hlog content can be replayed and put back into the
		// memstore.
		// Otherwise, the snapshot content while backed up in the hlog, it will
		// not
		...
		try {
			...
			LOG
					.debug("Caches flushed, doing commit now (which includes update scanners)");
			/**
			 * Switch between memstore and the new store file
			 */
			if (atomicWork != null) {
				LOG
						.debug("internalPreFlushcacheCommit gives us work to do, acquiring new scanner lock");
				newScannerLock.writeLock().lock();
			}
			try {
			...
			DroppedSnapshotException dse = new DroppedSnapshotException(
					"region: " + Bytes.toStringBinary(getRegionName()));
			dse.initCause(t);
			throw dse;
		}
		...
		this.log.completeCacheFlush(getRegionName(), regionInfo.getTableDesc()
				.getName(), completeSequenceId);
		...
		if (LOG.isDebugEnabled()) {
			...
			LOG.debug("Finished memstore flush of ~"
					+ StringUtils.humanReadableInt(currentMemStoreSize)
					+ " for region " + this + " in " + (now - startTime)
					+ "ms, sequence id=" + sequenceId
	 ...
	 * flush commit stage.
	 * 
	 * If a subclass wishes that an atomic update of their work and the flush
	 * commit stage happens, they should return a callable. The new scanner lock
	 * will be acquired and released.
	 * 
	 * @throws java.io.IOException
	 *             allow children to throw exception
	 */
	...
	// ////////////////////////////////////////////////////////////////////////////
	/**
	 * Return all the data for the row that matches <i>row</i> exactly, or the
	 * one that immediately preceeds it, at or immediately before <i>ts</i>.
	 * 
	 * @param row
	 *            row key
	...
	public Result getClosestRowBefore(final byte[] row, final byte[] family)
		...
		// closest key is across all column families, since the data may be
		// sparse
		KeyValue key = null;
		checkRow(row);
		splitsAndClosesLock.readLock().lock();
		try {
			...
			// This will get all results for this store. TODO: Do we need to do
			// this?
	 ...
	 *            delete object
	 * @param lockid
	 *            existing lock id, or null for grab a lock
	 * @param writeToWAL
	...
	void delete(byte[] family, List<KeyValue> edits, boolean writeToWAL)
			throws IOException {
		...
		try {
			if (writeToWAL) {
				for (Map.Entry<byte[], List<KeyValue>> e : familyMap.entrySet()) {
					...
					this.log.append(regionInfo.getRegionName(), regionInfo
							.getTableDesc().getName(), kvs, (regionInfo
							.isMetaRegion() || regionInfo.isRootRegion()), now);
				}
			}
			...
			for (Map.Entry<byte[], List<KeyValue>> e : familyMap.entrySet()) {
				...
				Store store = getStore(family);
				Map<byte[], Integer> kvCount = new TreeMap<byte[], Integer>(
				...
				for (KeyValue kv : kvs) {
					...
					if (kv.isLatestTimestamp() && kv.isDeleteType()) {
						...
						if (count == null) {
							...
							System.out.println("Deleting more than 1: "
									+ (count + 1));
						...
						Bytes.putBytes(kv.getBuffer(), kv.getTimestampOffset(),
								getkv.getBuffer(), getkv.getTimestampOffset(),
								Bytes.SIZEOF_LONG);
					} else {
						kv.updateLatestStamp(byteNow);
					}
	...
	public void put(Put put, Integer lockid, boolean writeToWAL)
			throws IOException {
		if (putTimes > 5000) {
			LOG.debug("region put if meta:"
					+ this.getRegionInfo().isMetaRegion());
			LOG.debug("region put if root:"
					+ this.getRegionInfo().isRootRegion());
			LOG.debug("region put if table is:"
					+ this.getRegionInfo().getTableDesc().getNameAsString());
			putTimes = 0;
		} else {
			putTimes++;
		}
		...
		try {
			...
			try {
				for (Map.Entry<byte[], List<KeyValue>> entry : put
	...
	/**
	 * Add updates first to the hlog (if writeToWal) and then add values to
	 * memstore. Warning: Assumption is caller has lock on passed in row.
	 * 
	 * @param familyMap
	 * @param writeToWAL
	 *            if true, then we should write to the log
	 * @throws IOException
	 */
	private void put(final Map<byte[], List<KeyValue>> familyMap,
		...
		try {
			...
			for (Map.Entry<byte[], List<KeyValue>> e : familyMap.entrySet()) {
				...
				if (writeToWAL) {
					this.log.append(regionInfo.getRegionName(), regionInfo
							.getTableDesc().getName(), edits, (regionInfo
							.isMetaRegion() || regionInfo.isRootRegion()), now);
				}
			}
	...
	protected void doReconstructionLog(Path oldLogFile, long minSeqId,
			long maxSeqId, Progressable reporter)
	...
	protected Store instantiateHStore(Path baseDir, HColumnDescriptor c,
			Path oldLogFile, Progressable reporter) throws IOException {
		return new Store(baseDir, this, c, this.fs, oldLogFile, this.conf,
				reporter);
	}

	/**
	 * Return HStore instance. Use with caution. Exposed for use of fixup
	 * utilities.
	 * 
	 * @param column
	 ...
	 * submit the name of a row and start writing to it; it must know the
	 * correct lockid, which matches the lock list in memory.
	 * 
	 * <p>
	...
	/**
	 * Returns existing row lock if found, otherwise obtains a new row lock and
	 * returns it.
	 * 
	 * @param lockid
	 * @return lockid
	...
	private void waitOnRowLocks() {
		synchronized (locksToRows) {
			while (this.locksToRows.size() > 0) {
				LOG.debug("waiting for " + this.locksToRows.size()
						+ " row locks");
				try {
	 ...
	 * <p>
	 * It is used to combine scanners from multiple Stores (aka column
	 * families).
	 */
	int tryTimes = 3;
	private boolean  checkClose()
	{
		...
		if(tryTime>=tryTimes)
		{
			HMsg msgs[]=new HMsg[1];
			HMsg message =new HMsg(HMsg.Type.MSG_NO_SERVER_REGION,regionInfo);
			msgs[0]=message;
			 try {
				 if(HRegionServer.instance!=null&&HRegionServer.instance.getMasterS()!=null)
				HRegionServer.instance.getMasterS().regionServerReport(
						HRegionServer.instance.serverInfo, msgs,
						HRegionServer.instance.getMostLoadedRegions());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	class RegionScanner implements InternalScanner {
		...
		void initHeap() {
			...
			for (Map.Entry<byte[], NavigableSet<byte[]>> entry : theScan
					.getFamilyMap().entrySet()) {
				Store store = stores.get(entry.getKey());
				scanners.add(store.getScanner(theScan, entry.getValue()));
			}
			this.storeHeap = new KeyValueHeap(scanners
					.toArray(new KeyValueScanner[0]), comparator);
		}
		...
		public boolean next(List<KeyValue> outResults) throws IOException {
			...
			{
				close();
			throw new NotServingRegionException(regionInfo
		...
		private boolean isStopRow(byte[] currentRow) {
			return currentRow == null
					|| (this.stopRow != null && comparator.compareRows(
							this.stopRow, 0, this.stopRow.length, currentRow,
							0, currentRow.length) <= isScan);
		}

		private boolean filterRow() {
			return (filter != null && filter.filterRow()) || oldFilter != null
					&& oldFilter.filterRow(results);
		}
		...
		private boolean filterRowKey(byte[] row) {
			return (this.filter != null && this.filter.filterRowKey(row, 0,
					row.length))
					|| (oldFilter != null && this.oldFilter.filterRowKey(row,
							0, row.length));
		}
	 ...
	 */
	public static HRegion newHRegion(Path basedir, HLog log, FileSystem fs,
			HBaseConfiguration conf, HRegionInfo regionInfo,
			FlushRequester flushListener) {
		try {
			@SuppressWarnings("unchecked")
			Class<? extends HRegion> regionClass = (Class<? extends HRegion>) conf
					.getClass(HConstants.REGION_IMPL, HRegion.class);

			Constructor<? extends HRegion> c = regionClass.getConstructor(
					Path.class, HLog.class, FileSystem.class,
					HBaseConfiguration.class, HRegionInfo.class,
					FlushRequester.class);

			return c.newInstance(basedir, log, fs, conf, regionInfo,
					flushListener);
		} catch (Throwable e) {
			// todo: what sould I throw here?
			throw new IllegalStateException(
					"Could not instantiate a region instance.", e);
		}
	}

	/**
	 * Convenience method creating new HRegions. Used by createTable and by the
	 * bootstrap code in the HMaster constructor. Note, this method creates an
	 * {@link HLog} for the created region. It needs to be closed explicitly.
	 * Use {@link HRegion#getLog()} to get access.
	 * 
	 * @param info
	 *            Info for region to create.
	 * @param rootDir
	...
	public static HRegion createHRegion(final HRegionInfo info,
			final Path rootDir, final HBaseConfiguration conf)
			throws IOException {
		Path tableDir = HTableDescriptor.getTableDir(rootDir, info
				.getTableDesc().getName());
		Path regionDir = HRegion.getRegionDir(tableDir, info.getEncodedName());
		FileSystem fs = FileSystem.get(conf);
		fs.mkdirs(regionDir);
		HRegion region = HRegion.newHRegion(tableDir, new HLog(fs, new Path(
				regionDir, HREGION_LOGDIR_NAME), conf, null), fs, conf, info,
				null);
	 ...
	 *            HRegion#getMinSequenceId() to ensure the log id is properly
	 *            kept up. HRegionStore does this every time it opens a new
	 *            region.
	 * @param conf
	 * @return new HRegion
	 * 
	...
	public static HRegion openHRegion(final HRegionInfo info,
			final Path rootDir, final HLog log, final HBaseConfiguration conf)
			throws IOException {
		if (LOG.isDebugEnabled()) {
		...
		HRegion r = HRegion.newHRegion(HTableDescriptor.getTableDir(rootDir,
				info.getTableDesc().getName()), log, FileSystem.get(conf),
				conf, info, null);
	...
	/**
	 * Inserts a new region's meta information into the passed <code>meta</code>
	 * region. Used by the HMaster bootstrap code adding new table to ROOT
	 * table.
	 * 
	 * @param meta
	 *            META HRegion to be updated
	 * @param r
	 *            HRegion to add to <code>meta</code>
	 * 
	 * @throws IOException
	 */
	public static void addRegionToMETA(HRegion meta, HRegion r)
		...
		try {
			...
			edits.add(new KeyValue(row, CATALOG_FAMILY, REGIONINFO_QUALIFIER,
					System.currentTimeMillis(), Writables.getBytes(r
							.getRegionInfo())));
			Map<byte[], List<KeyValue>> familyMap = new TreeMap<byte[], List<KeyValue>>();
	...
	/**
	 * Delete a region's meta information from the passed <code>meta</code>
	 * region. Removes content in the 'info' column family. Does not remove
	 * region historian info.
	 * 
	 * @param srvr
	 *            META server to be updated
	 * @param metaRegionName
	 *            Meta region name
	 * @param regionName
	 *            HRegion to remove from <code>meta</code>
	 * 
	 * @throws IOException
	 */
	 ...
	 *            Meta region name
	 * @param info
	 *            HRegion to update in <code>meta</code>
	 * 
	 * @throws IOException
	 */
	 ...
	 * @throws IOException
	 */
	public static void deleteRegion(FileSystem fs, Path rootdir,
	...
	public static Path getRegionDir(final Path rootdir, final HRegionInfo info) {
		return new Path(HTableDescriptor.getTableDir(rootdir, info
				.getTableDesc().getName()), Integer.toString(info
				.getEncodedName()));
	}
	...
	public static boolean rowIsInRange(HRegionInfo info, final byte[] row) {
		return ((info.getStartKey().length == 0) || (Bytes.compareTo(info
				.getStartKey(), row) <= 0))
				&& ((info.getEndKey().length == 0) || (Bytes.compareTo(info
						.getEndKey(), row) > 0));
	}
	...
	public static void makeColumnFamilyDirs(FileSystem fs, Path tabledir,
		...
		Path dir = Store.getStoreHomedir(tabledir, hri.getEncodedName(),
				colFamily);
		if (!fs.mkdirs(dir)) {
		...
		if (srcA.getStartKey() == null) {
			if (srcB.getStartKey() == null) {
				throw new IOException(
						"Cannot merge two regions with null start key");
			}
	...
	public static HRegion merge(HRegion a, HRegion b) throws IOException {
		...
		final byte[] startKey = a.comparator.matchingRows(a.getStartKey(), 0, a
				.getStartKey().length, EMPTY_BYTE_ARRAY, 0,
				EMPTY_BYTE_ARRAY.length)
				|| b.comparator.matchingRows(b.getStartKey(), 0, b
						.getStartKey().length, EMPTY_BYTE_ARRAY, 0,
						EMPTY_BYTE_ARRAY.length) ? EMPTY_BYTE_ARRAY
				: a.comparator.compareRows(a.getStartKey(), 0,
						a.getStartKey().length, b.getStartKey(), 0, b
								.getStartKey().length) <= 0 ? a.getStartKey()
						: b.getStartKey();
		final byte[] endKey = a.comparator.matchingRows(a.getEndKey(), 0, a
				.getEndKey().length, EMPTY_BYTE_ARRAY, 0,
				EMPTY_BYTE_ARRAY.length)
				|| a.comparator.matchingRows(b.getEndKey(), 0,
						b.getEndKey().length, EMPTY_BYTE_ARRAY, 0,
						EMPTY_BYTE_ARRAY.length) ? EMPTY_BYTE_ARRAY
				: a.comparator.compareRows(a.getEndKey(), 0,
						a.getEndKey().length, b.getEndKey(), 0,
						b.getEndKey().length) <= 0 ? b.getEndKey() : a
						.getEndKey();
		...
		LOG.info("starting merge of regions: " + a + " and " + b
				...
				+ " with start key <" + Bytes.toString(startKey)
				+ "> and end key <" + Bytes.toString(endKey) + ">");

		// Move HStoreFiles under new region directory
		Map<byte[], List<StoreFile>> byFamily = new TreeMap<byte[], List<StoreFile>>(
				Bytes.BYTES_COMPARATOR);
		...
		for (Map.Entry<byte[], List<StoreFile>> es : byFamily.entrySet()) {
			...
			// Because we compacted the source regions we should have no more
			// than two
			...
			if (srcFiles.size() == 2) {
				...
				if (seqA == seqB) {
					// Can't have same sequenceid since on open of a store, this
					// is what
					// distingushes the files (see the map of stores how its
					// keyed by
					// sequenceid).
			...
			for (StoreFile hsf : srcFiles) {
				StoreFile.rename(fs, hsf.getPath(), StoreFile.getUniqueFile(fs,
						Store.getStoreHomedir(basedir, newRegionInfo
								.getEncodedName(), colFamily)));
			}
		}
		if (LOG.isDebugEnabled()) {
		...
		HRegion dstRegion = HRegion.newHRegion(basedir, log, fs, conf,
				newRegionInfo, null);
	 ...
	 *            get object
	 * @param ignoredLockId
	 *            lock id that is ignored
	 * @return result
	 * @throws IOException
	 *             read exceptions
	 */
	static int getTimes = 0;
	static int putTimes = 0;

	public Result get(final Get get, final Integer lockid) throws IOException {
		// Verify families are all valid
		if (getTimes > 1000) {
			LOG.debug("region get if meta:"
					+ this.getRegionInfo().isMetaRegion());
			LOG.debug("region get if root:"
					+ this.getRegionInfo().isRootRegion());
			LOG.debug("region get if table is:"
					+ this.getRegionInfo().getTableDesc().getNameAsString());
			getTimes = 0;
		}
	...
	public long incrementColumnValue(byte[] row, byte[] family,
		...
		try {
			...
			get.addColumn(family, qualifier);
			List<KeyValue> results = new ArrayList<KeyValue>();
			NavigableSet<byte[]> qualifiers = new TreeSet<byte[]>(
					Bytes.BYTES_COMPARATOR);
			...
			KeyValue newKv = new KeyValue(row, family, qualifier, System
					.currentTimeMillis(), Bytes.toBytes(result));
			...
			// appropriately depending on if there is a value in memcache or
			// not.
			// returns the
	...
	public static final long FIXED_OVERHEAD = ClassSize
			.align((5 * Bytes.SIZEOF_LONG) + Bytes.SIZEOF_BOOLEAN
	...
	/*
	 * Process table. Do major compaction or list content.
	 * 
	 * @param fs
	 * 
	 * @param p
	...
	private static void processTable(final FileSystem fs, final Path p,
		...
		if (p.getName().startsWith(rootStr)) {
			...
			region = HRegion.newHRegion(p, log, fs, c,
					HRegionInfo.FIRST_META_REGIONINFO, null);
		} else {
	 ...
	 * tables since these are only tables we for sure know schema on. For usage
	 * run:
	 * 
	 * <pre>
	...
	public static void main(String[] args) throws IOException {
		...
		Path logdir = new Path(c.get("hbase.tmp.dir"), "hlog"
				+ tableDir.getName() + System.currentTimeMillis());
