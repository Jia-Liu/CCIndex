...
public class HRegionServer implements HConstants, HRegionInterface,
	...
	private static final HMsg REPORT_QUIESCED = new HMsg(
			Type.MSG_REPORT_QUIESCED);
	private static final HMsg[] EMPTY_HMSG_ARRAY = new HMsg[] {};
	public static HRegionServer instance=null;
	 ...
	 * recover from an OOME. See HBASE-706. TODO: Make this percentage of the
	 * heap or a minimum.
	 */
	...
	public HRegionServer(HBaseConfiguration conf) throws IOException {
		...
		String addressStr = machineName
				+ ":"
				+ conf.get(REGIONSERVER_PORT, Integer
						.toString(DEFAULT_REGIONSERVER_PORT));
		...
		this.threadWakeFrequency = conf
				.getInt(THREAD_WAKE_FREQUENCY, 10 * 1000);
		this.msgInterval = conf.getInt("hbase.regionserver.msginterval",
				1 * 1000);
		this.serverLeaseTimeout = conf.getInt("hbase.master.lease.period",
				120 * 1000);
		...
		this.rpcTimeout = conf
				.getLong("hbase.regionserver.lease.period", 60000);
	...
	private void reinitialize() throws IOException {
		...
		this.server = HBaseRPC.getServer(this, address.getBindAddress(),
				address.getPort(), conf.getInt(
						"hbase.regionserver.handler.count", 10), false, conf);
		this.server.setErrorHandler(this);
		// Address is giving a default IP for the moment. Will be changed after
		// calling the master.
		this.serverInfo = new HServerInfo(new HServerAddress(
				new InetSocketAddress(address.getBindAddress(), this.server
						.getListenerAddress().getPort())), System
				.currentTimeMillis(), this.conf.getInt(
	...
	private void reinitializeThreads() {
		...
		// Log flushing thread
		this.hlogFlusher = new LogFlusher(this.threadWakeFrequency,
				this.stopRequested);

		// Background thread to check for major compactions; needed if region
		// has not gotten updates in a while. Make it run at a lesser frequency.
		int multiplier = this.conf.getInt(
				THREAD_WAKE_FREQUENCY + ".multiplier", 1000);
		this.majorCompactionChecker = new MajorCompactionChecker(this,
				this.threadWakeFrequency * multiplier, this.stopRequested);

		this.leases = new Leases(conf.getInt("hbase.regionserver.lease.period",
				60 * 1000), this.threadWakeFrequency);
	}
	 ...
	 * 
	 * @param event
	 *            WatchedEvent from ZooKeeper.
	 */
	public void process(WatchedEvent event) {
		EventType type = event.getType();
		KeeperState state = event.getState();
		LOG.info("Got ZooKeeper event, state: " + state + ", type: " + type
				+ ", path: " + event.getPath());
		...
		if (state == KeeperState.Expired) {
			...
			boolean restart = this.conf.getBoolean(
					"hbase.regionserver.restart.on.zk.expire", false);
			if (restart) {
				restart();
			} else {
				abort();
			...
			// ZooKeeper watches are one time only, so we need to re-register
			// our watch.
			watchMasterAddress();
		}
	}
	...
	private void watchMasterAddress() {
		while (!stopRequested.get()
				&& !zooKeeperWrapper.watchMasterAddress(this)) {
			LOG
					.warn("Unable to set watcher on ZooKeeper master address. Retrying.");
			sleeper.sleep();
		}
	}
	...
	public void run() {
		...
		try {
			...
			while (!stopRequested.get()) {
				...
				sleeper.sleep();
				LOG
						.warn("No response from master on reportForDuty. Sleeping and "
								+ "then trying again.");
			}
			...
			for (int tries = 0; !stopRequested.get() && isHealthy();) {
				...
				if (!haveRootRegion.get()) {
					HServerAddress rootServer = zooKeeperWrapper
							.readRootRegionLocation();
					if (rootServer != null) {
						// By setting the root region location, we bypass the
						// wait imposed on
						// HTable for all regions being assigned.
						this.connection
								.setRootRegionLocation(new HRegionLocation(
				...
				if (lastMsg != 0 && (now - lastMsg) >= serverLeaseTimeout) {
					// It has been way too long since we last reported to the
					// master.
					LOG.warn("unable to report to master for "
							+ (now - lastMsg) + " milliseconds - retrying");
				}
				// Drop into the send loop if msgInterval has elapsed or if
				// something
				// to send. If we fail talking to the master, then we'll sleep
				// below
				...
				if ((now - lastMsg) >= msgInterval
					...
					try {
						doMetrics();
						MemoryUsage memory = ManagementFactory
						...
						HMsg msgs[] = this.hbaseMaster.regionServerReport(
								serverInfo, outboundMessages
										.toArray(EMPTY_HMSG_ARRAY),
								getMostLoadedRegions());
						lastMsg = System.currentTimeMillis();
						updateOutboundMsgs(outboundMessages);
						outboundMessages.clear();
						if (this.quiesced.get() && onlineRegions.size() == 0) {
							// We've just told the master we're exiting because
							// we aren't
							// serving any regions. So set the stop bit and
							// exit.
							LOG
									.info("Server quiesced and not serving any regions. "
											+ "Starting shutdown");
						...
						// Queue up the HMaster's instruction stream for
						// processing
						boolean restart = false;
						for (int i = 0; !restart && !stopRequested.get()
								&& i < msgs.length; i++) {
							...
							case MSG_CALL_SERVER_STARTUP:
								// We the MSG_CALL_SERVER_STARTUP on startup but
								// we can also
								// get it when the master is panicking because
								// for instance
								// the HDFS has been yanked out from under it.
								// Be wary of
								// this message.
								if (checkFileSystem()) {
									closeAllRegions();
									try {
										...
										LOG
												.error(
														"error closing and deleting HLog",
														e);
									}
									try {
										serverInfo.setStartCode(System
												.currentTimeMillis());
										hlog = setupHLog();
										this.hlogFlusher.setHLog(hlog);
									} catch (IOException e) {
										/**
										 *  a datanode is break down and cause hlog write error
										 */
									...
									LOG
											.fatal("file system available check failed. "
						...
						if (this.stopRequested.get()) {
							LOG
									.info("Stop requested, clearing toDo despite exception");
							toDo.clear();
							continue;
						}
						LOG.warn("Attempt=" + tries, e);
						// No point retrying immediately; this is probably
						// connection to
				...
				HMsg msg = this.outboundMsgs.poll(
						(msgInterval - (now - lastMsg)), TimeUnit.MILLISECONDS);
		...
		if (abortRequested) {
			...
			try {
				...
				LOG
						.info("telling master that region server is shutting down at: "
								+ serverInfo.getServerAddress().toString());
		...
		if (shutdownHDFS.get()) {
			runThread(this.hdfsShutdownThread, this.conf.getLong(
					"hbase.dfs.shutdown.wait", 30000));
		}
	...
	/**
	 * Set the hdfs shutdown thread to run on exit. Pass null to disable running
	 * of the shutdown test. Needed by tests.
	 * 
	 * @param t
	...
	protected void init(final MapWritable c) throws IOException {
		try {
			...
			// path with file system specification included. Set
			// 'fs.default.name'
			// to match the filesystem on hbase.rootdir else underlying hadoop
			// hdfs
			// accessors will be going against wrong filesystem (unless all is
			// set
			// to defaults).
			this.conf.set("fs.default.name", this.conf.get("hbase.rootdir"));
			this.fs = FileSystem.get(this.conf);

			// Register shutdown hook for HRegionServer, runs an orderly
			// shutdown
			// when a kill signal is recieved
			Runtime.getRuntime().addShutdownHook(
					new ShutdownThread(this, Thread.currentThread()));
	...
	public HServerLoad.RegionLoad createRegionLoad(final byte[] regionName) {
		return createRegionLoad(this.onlineRegions
				.get(Bytes.mapKey(regionName)));
	}
	 ...
	 * @param t
	 * 
	 * @param msg Message to put in new IOE if passed <code>t</code> is not an
	 * IOE
	 * 
	 * @return Make <code>t</code> an IOE if it isn't already.
	 */
	private IOException convertThrowableToIOE(final Throwable t,
		...
		return (t instanceof IOException ? (IOException) t : msg == null
				|| msg.length() == 0 ? new IOException(t) : new IOException(
				msg, t));
	}
	...
	/*
	 * Thread to shutdown the region server in an orderly manner. This thread is
	 * registered as a shutdown hook in the HRegionServer constructor and is
			...
			for (Integer i : keys) {
				...
				try {
					if (r != null && r.isMajorCompaction()) {
						// Queue a compaction. Will recognize if major is
						// needed.
						this.instance.compactSplitThread.compactionRequested(r,
	 ...
	 * so that all filesystems can be closed when the process is terminated.
	 * This conveniently runs concurrently with our own shutdown handler, and
	 * therefore causes all the filesystems to be closed before the server can
	 * do all its necessary cleanup.
	 * 
	 * The crazy dirty reflection in this method sneaks into the FileSystem
	 * cache and grabs the shutdown hook, removes it from the list of active
	 * shutdown hooks, and hangs onto it until later. Then, after we're properly
	 * done with our graceful shutdown, we can execute the hdfs hook manually to
	...
	private Thread suppressHdfsShutdownHook() {
		try {
			...
			if (hdfsClientFinalizer == null) {
				throw new RuntimeException(
						"client finalizer is null, can't suppress!");
			}
			Runtime.getRuntime().removeShutdownHook(hdfsClientFinalizer);
			return hdfsClientFinalizer;

		} catch (NoSuchFieldException nsfe) {
			LOG.fatal("Couldn't find field 'clientFinalizer' in FileSystem!",
					nsfe);
			throw new RuntimeException("Failed to suppress HDFS shutdown hook");
		} catch (IllegalAccessException iae) {
			LOG.fatal("Couldn't access field 'clientFinalizer' in FileSystem!",
					iae);
			throw new RuntimeException("Failed to suppress HDFS shutdown hook");
		}
	}
	 ...
	 * is completed (setting up filesystem, starting service threads, etc.).
	 * This method is designed mostly to be useful in tests.
	 * 
	...
	private HLog setupHLog() throws RegionServerRunningException, IOException {
		...
		Path logdir = new Path(rootDir, HLog
				.getHLogDirectoryName(this.serverInfo));
		if (LOG.isDebugEnabled()) {
	...
	protected void metrics() {
		...
		// Is this too expensive every three seconds getting a lock on
		// onlineRegions
	 ...
	 * Install an UncaughtExceptionHandler that calls abort of RegionServer if
	 * we get an unhandled exception. We cannot set the handler on all threads.
	 ...
	 * Chore, it keeps its own internal stop mechanism so needs to be stopped by
	 * this hosting server. Worker logs the exception and exits.
	 */
	private void startServiceThreads() throws IOException {
		...
		Threads.setDaemonThreadRunning(this.cacheFlusher, n + ".cacheFlusher",
				handler);
		Threads.setDaemonThreadRunning(this.compactSplitThread, n
				+ ".compactor", handler);
		Threads.setDaemonThreadRunning(this.workerThread, n + ".worker",
				handler);
		Threads.setDaemonThreadRunning(this.majorCompactionChecker, n
				+ ".majorCompactionChecker", handler);

		// Leases is not a Thread. Internally it runs a daemon thread. If it
		// gets
		...
		if (port >= 0) {
			...
			boolean auto = this.conf.getBoolean(
					"hbase.regionserver.info.port.auto", false);
			while (true) {
				try {
					this.infoServer = new InfoServer("regionserver", addr,
							port, false);
					...
					LOG
							.info("Failed binding http info server to port: "
									+ port);
					port++;
	...
	private boolean isHealthy() {
		...
		if (!(leases.isAlive() && compactSplitThread.isAlive()
				...
				&& workerThread.isAlive() && this.majorCompactionChecker
				.isAlive())) {
	 ...
	 * log it is using and without notifying the master. Used unit testing and
	 * on catastrophic events such as HDFS is yanked out from under hbase or we
	 * OOME.
	 */
	public void abort() {
	...
	HMasterRegionInterface getMasterS()
	{
	...
	private boolean getMaster() {
		...
		while (masterAddress == null) {
			...
			try {
				...
				LOG.warn(
						"Unable to read master address from ZooKeeper. Retrying."
								+ " Error was:", e);
				sleeper.sleep();
			}
		}
		...
		while (!stopRequested.get() && master == null) {
			try {
				// Do initial RPC setup. The final argument indicates that the
				// RPC
				// should retry indefinitely.
				master = (HMasterRegionInterface) HBaseRPC.waitForProxy(
						HMasterRegionInterface.class,
						HBaseRPCProtocolVersion.versionID, masterAddress
								.getInetSocketAddress(), this.conf, -1,
						this.rpcTimeout);
			} catch (IOException e) {
				LOG
						.warn(
		...
		while (!stopRequested.get()) {
			try {
				...
				HServerLoad hsl = new HServerLoad(0,
						(int) memory.getUsed() / 1024 / 1024, (int) memory
								.getMax() / 1024 / 1024);
				this.serverInfo.setLoad(hsl);
				if (LOG.isDebugEnabled())
				...
				while (!startCodeOk) {
					...
					startCodeOk = zooKeeperWrapper
							.writeRSLocation(this.serverInfo);
					if (!startCodeOk) {
						LOG
								.debug("Start code already taken, trying another one");
					}
				}
	...
	private void reportClose(final HRegionInfo region, final byte[] message) {
		this.outboundMsgs.add(new HMsg(HMsg.Type.MSG_REPORT_CLOSE, region,
				message));
	}
	...
	void reportSplit(HRegionInfo oldRegion, HRegionInfo newRegionA,
		...
		this.outboundMsgs.add(new HMsg(
				HMsg.Type.MSG_REPORT_SPLIT_INCLUDES_DAUGHTERS, oldRegion,
				newRegionA, newRegionB, Bytes.toBytes("Daughters; "
		...
		public void run() {
			try {
				while (!stopRequested.get()) {
					ToDoEntry e = null;
					try {
						e = toDo.poll(threadWakeFrequency,
						...
						case MSG_REGION_OPEN:
							...
							if (!haveRootRegion.get() && !info.isRootRegion()) {
								// root region is not online yet. requeue this
								// task
								LOG
										.info("putting region open request back into queue because"
								...
								try {
									...
									LOG
											.warn(
													"insertion into toDo queue was interrupted",
													ex);
									break;
								}
							}
						...
						case MSG_REGION_COMPACT:
							...
							compactSplitThread
									.compactionRequested(
											region,
											e.msg
													.isType(Type.MSG_REGION_MAJOR_COMPACT),
											e.msg.getType().name());
							break;
						...
						if (ex instanceof IOException) {
							ex = RemoteExceptionHandler
									.checkIOException((IOException) ex);
						}
	...
	void openRegion(final HRegionInfo regionInfo) {
		...
		if (region == null) {
			try {
				...
				// Startup a compaction early if one is needed, if region has
				// references
				// or if a store has too many store files
				if (region.hasReferences() || region.hasTooManyStoreFiles()) {
					this.compactSplitThread.compactionRequested(region, region
							.hasReferences() ? "Region has references on open"
							: "Region has too many store files");
				}
			} catch (Throwable e) {
				Throwable t = cleanup(e, "Error opening "
						+ regionInfo.getRegionNameAsString());
				// TODO: add an extra field in HRegionInfo to indicate that
				// there is
				// an error. We can't do that now because that would be an
				// incompatible
				// change that would require a migration
				reportClose(regionInfo, StringUtils.stringifyException(t)
						.getBytes());
				return;
			}
	...
	protected HRegion instantiateRegion(final HRegionInfo regionInfo)
		...
		HRegion r = HRegion.newHRegion(HTableDescriptor.getTableDir(rootDir,
				regionInfo.getTableDesc().getName()), this.hlog, this.fs, conf,
				regionInfo, this.cacheFlusher);
	...
	/**
	 * Add a MSG_REPORT_PROCESS_OPEN to the outbound queue. This method is
	 * called while region is in the queue of regions to process and then while
	 * the region is being opened, it is called from the Worker thread that is
	 * running the region open.
	 * 
	 * @param hri
	 *            Region to add the message for
	 */
	public void addProcessingMessage(final HRegionInfo hri) {
		getOutboundMsgs().add(new HMsg(HMsg.Type.MSG_REPORT_PROCESS_OPEN, hri));
	}

	protected void closeRegion(final HRegionInfo hri,
			final boolean reportWhenCompleted) throws IOException {
	...
	private static class RegionCloserThread extends Thread {
		...
		protected RegionCloserThread(final HRegion r) {
			super(Thread.currentThread().getName() + ".regionCloser."
					+ r.toString());
			this.r = r;
		}
	...
	void closeUserRegions() {
		...
		try {
			synchronized (onlineRegions) {
				for (Iterator<Map.Entry<Integer, HRegion>> i = onlineRegions
	...
	public SortedMap<Long, HRegion> getCopyOfOnlineRegionsSortedBySize() {
		...
		synchronized (this.onlineRegions) {
			for (HRegion region : this.onlineRegions.values()) {
				sortedRegions.put(Long.valueOf(region.memstoreSize.get()),
						region);
			}
		}
	...
	protected HRegion getRegion(final byte[] regionName)
		...
		try {
			region = onlineRegions.get(Integer.valueOf(Bytes
					.hashCode(regionName)));
			if (region == null) {
				HMsg msgs[]=new HMsg[1];
				HMsg message =new HMsg(HMsg.Type.MSG_NO_SERVER_REGION,new HRegionInfo().setRegionName(regionName));
				msgs[0]=message;
				 try {
					this.hbaseMaster.regionServerReport(
							serverInfo, msgs,
							getMostLoadedRegions());
				} catch (IOException e) {
	 ...
	 * the master which regions it can reallocate if we're overloaded. TODO:
	 * actually calculate which regions are most loaded. (Right now, we're just
	 * grabbing the first N regions being served regardless of load.)
	 */
	...
	private static void printUsageAndExit(final String message) {
		...
		System.err
				.println("Usage: java org.apache.hbase.HRegionServer start|stop");
		System.exit(0);
	}
				...
				try {
					...
					if (LocalHBaseCluster.isLocal(conf)) {
						...
						Constructor<? extends HRegionServer> c = regionServerClass
								.getConstructor(HBaseConfiguration.class);
						HRegionServer hrs = c.newInstance(conf);
						if(HRegionServer.instance==null)
						{
							HRegionServer.instance=hrs;
						}
						Thread t = new Thread(hrs);
						t.setName("regionserver"
								+ hrs.server.getListenerAddress());
						t.start();
					}
				} catch (Throwable t) {
