...
public class Store implements HConstants, HeapSize {
  ...
  private HFile.Writer getWriter(final Path basedir) throws IOException {
	  short replication=3;
	  if(this.getHRegionInfo().getTableDesc().getNameAsString().
			  endsWith(new String(CCIndexAdmin.CCT_TAIL)))
			  {
		  		replication=3;
			  }
	  if(this.getHRegionInfo().isMetaRegion())
			  {
		  		replication=3;
			  }
	  if(this.getHRegionInfo().isRootRegion())
	  {
  		replication=3; 
	  }
	  if(this.getHRegionInfo().getTableDesc().getNameAsString().
			  endsWith(new String(CCIndexAdmin.INDEX_TAIL)))
			  {
		  		replication=1;
			  }
	   
    return StoreFile.getWriter(this.fs, basedir, this.blocksize,
        this.compression, this.comparator.getRawComparator(),replication);
  }
