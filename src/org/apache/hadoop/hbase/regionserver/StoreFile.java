...
/**
 * A Store data file. Stores usually have one or more of these files. They are
 * produced by flushing the memstore to disk. To create, call
 * {@link #getWriter(FileSystem, Path)} and append data. Be sure to add any
 * metadata before calling close on the Writer (Use the appendMetadata
 * convenience methods). On close, a StoreFile is sitting in the Filesystem. To
 * refer to it, create a StoreFile instance passing filesystem and path. To
 * read, call {@link #getReader()}.
 * <p>
...
public class StoreFile implements HConstants {
	...
	private static final byte[] MAX_SEQ_ID_KEY = Bytes
			.toBytes("MAX_SEQ_ID_KEY");
	// Set when we obtain a Reader.
	private long sequenceid = -1;

	private static final byte[] MAJOR_COMPACTION_KEY = Bytes
			.toBytes("MAJOR_COMPACTION_KEY");
	 ...
	 * @param blockcache
	 *            <code>true</code> if the block cache is enabled.
	 * @param conf
	 *            The current configuration.
	 * @throws IOException
	 *             When opening the reader fails.
	 */
	 ...
	 * hierarchy of
	 * <code>${hbase.rootdir}/tablename/regionname/familyname</code>.
	 * 
	...
	static Path getReferredToFile(final Path p) {
		...
		return new Path(new Path(new Path(tableDir, otherRegion), p.getParent()
	...
	protected HFile.Reader open() throws IOException {
		...
		if (b != null) {
			// By convention, if halfhfile, top half has a sequence number >
			// bottom
			// half. Thats why we add one in below. Its done for case the two
			// halves
			// are ever merged back together --rare. Without it, on open of
			// store,
			// since store files are distingushed by sequence id, the one half
			// would
			// subsume the other.
		...
		// TODO read in bloom filter here, ignore if the column family config
		// says
	...
	public String toString() {
		return this.path.toString()
				+ (isReference() ? "-" + this.referencePath + "-"
						+ reference.toString() : "");
	}
	...
	public static Path rename(final FileSystem fs, final Path src,
			final Path tgt) throws IOException {
		if (!fs.exists(src)) {
	...
	/**
	 * Get a store file writer. Client is responsible for closing file when
	 ...
	 * @param fs
	 * @param dir
	 *            Path to family directory. Makes the directory if doesn't
	 *            exist. Creates a file with a unique name in this directory.
	 * @return HFile.Writer
	 * @throws IOException
	 */
	...
	public static HFile.Writer getWriter(final FileSystem fs, final Path dir,
			...
			final KeyValue.KeyComparator c) throws IOException {
		if (!fs.exists(dir)) {
			fs.mkdirs(dir);
		}
		...
		return new HFile.Writer(fs, path, blocksize,
				algorithm == null ? HFile.DEFAULT_COMPRESSION_ALGORITHM
						: algorithm, c == null ? KeyValue.KEY_COMPARATOR : c);
	}

	public static HFile.Writer getWriter(final FileSystem fs, final Path dir,
			final int blocksize, final Compression.Algorithm algorithm,
			final KeyValue.KeyComparator c, short replica) throws IOException {
		if (!fs.exists(dir)) {
			fs.mkdirs(dir);
		}
		...
		try {
			writer = new HFile.Writer(fs, path, blocksize,
					algorithm == null ? HFile.DEFAULT_COMPRESSION_ALGORITHM
							: algorithm, c == null ? KeyValue.KEY_COMPARATOR
							: c, replica);
		} catch (Throwable e) {
			// TODO Auto-generated catch block
			System.err
					.println("!!!!!!!!!!!!!!!!!!get writer error some thing most be done here!");
			e.printStackTrace();
		}
		return writer;
	}
	...
	/**
	 * Write file metadata. Call before you call close on the passed
	 * <code>w</code> since its written as metadata to that file.
	 * 
	 * @param w
	 *            hfile writer
	 * @param maxSequenceId
	...
	/**
	 * Writes metadata. Call before you call close on the passed <code>w</code>
	 * since its written as metadata to that file.
	 * 
	 * @param maxSequenceId
	 *            Maximum sequence id.
	 * @param mc
	 ...
	 * @throws IOException
	 */
	public static void appendMetadata(final HFile.Writer w,
			final long maxSequenceId, final boolean mc) throws IOException {
	...
	static Path split(final FileSystem fs, final Path splitDir,
			final StoreFile f, final byte[] splitRow,
			final Reference.Range range) throws IOException {
		// A reference to the bottom half of the hsf store file.
		Reference r = new Reference(splitRow, range);
		// Add the referred-to regions name as a dot separated suffix.
		// See REF_NAME_PARSER regex above. The referred-to regions name is
		// up in the path of the passed in <code>f</code> -- parentdir is
		// family,
