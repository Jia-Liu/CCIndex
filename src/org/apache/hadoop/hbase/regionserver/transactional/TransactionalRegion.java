...
static long getTimes=0;
  public Result get(final long transactionId, Get get) throws IOException {
	  if(getTimes>1000)
	  {
		LOG.debug("region get if meta:"+this.getRegionInfo().isMetaRegion());
	  	LOG.debug("region get if root:"+this.getRegionInfo().isRootRegion());
	  	LOG.debug("region get if table is:"+this.getRegionInfo().getTableDesc().getNameAsString());
	  	getTimes=0;
	  }
	  getTimes++;
    checkClosing();
   ...
   * @throws IOException
   */
  static long putTimes=0;
  public void put(final long transactionId, final Put put) throws IOException {
	  if(putTimes>5000)
	  {
		LOG.debug("region put if meta:"+this.getRegionInfo().isMetaRegion());
	  	LOG.debug("region put if root:"+this.getRegionInfo().isRootRegion());
	  	LOG.debug("region put if table is:"+this.getRegionInfo().getTableDesc().getNameAsString());
	  	putTimes=0;
	  }putTimes++;
	  checkClosing();
    super.put(put);
